...
| `stream_keepalive_interval_seconds` | integer | `0` | Interval between SSE keepalive frames (a `: keepalive` comment) on idle streams. Keeps long-idle streams alive through idle-timeout enforcing intermediaries (load balancers, reverse proxies). `0` disables. Set below the smallest idle timeout on the path. |
